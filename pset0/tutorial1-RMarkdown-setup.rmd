...
In this tutorial, we will be using [RStudio](https://www.rstudio.com/) to interact with R, and write annotated R code using [Markdown](http://rmarkdown.rstudio.com).
...
If you want to disable inline outputs of your code chunks and instead display them in `Console` and `Plots` panes, you can make the following change:

Go to `Tools` -> `Global Options...` -> `R Markdown` and unselect `Show output inline for all R Markdown documents`
